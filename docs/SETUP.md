...
| `--sample` | 隨機抽 10 則看看 |
| `--as-of YYYY-MM-DD` | 把「今天」假裝成這一天，只能配合 `--dry-run` 或 `--show` |
...
| `POST_BANNED` | （空）| 平常封殺不讓它出現的排列（不含字尾），逗號分隔可列多個 |
...
| `{holiday_name}` | 下一個連假的名字，已經是「OO連假」的完整說法（查 `calendar_tw.next_long_weekend`） |
| `{holiday_days}` | 距離**連假第一天**還有幾天 |

`holiday_days` 算的是整段連續休息日的起點，不是那個假日本身一天的——
台灣人講「中秋連假」，指的是連著的週休二日也算進去的那幾天（教師節
是週一，但連假從前面的週六就開始）。名字透過 `HOLIDAY_SHORT_NAMES`
換成台灣人慣用的簡稱（國慶日→雙十連假、教師節維持教師節連假），
...
設定 `PRE_BREAK_TEXT` 之後，今天上班、明天就是連假第一天的那一天
會發這句固定內容，取代平常的隨機抽籤（例如把平常的字尾換成驚嘆號，
特別版跟一般版一望即知）。判斷用的是 `calendar_tw.next_long_weekend`，
跟回覆的倒數共用同一套邏輯，兩者的「還剩幾天」永遠一致。
...
`main()` 判斷「今天」用的是系統實際日期，補假、連假前一天這種
日期相關的特例，平常沒到那天就測不到。`--as-of` 讓你把「今天」
假裝成任何一天，配合 `--dry-run` 就能提前看到那天實際會發的內容：

```
python post_sunrise.py --as-of 2026-09-24 --dry-run
```

為了不小心用假日期真的發文，`--as-of` 只能配合 `--dry-run` 或
`--show` 使用，其他情況會直接報錯。這個測法讀的是本機 `.env`，
GitHub secret 的內容要另外貼進 `.env` 對應的欄位才測得到，
兩邊是各自獨立的設定，不會互相同步。

政府每年公布新的行事曆時，若出現表裡沒有的新假日名稱，會自動退回
使用原始名稱（不會壞掉），但可能不符合台灣人的慣用簡稱，屆時要
手動把新名稱加進 `calendar_tw.py` 的 `HOLIDAY_SHORT_NAMES`。
