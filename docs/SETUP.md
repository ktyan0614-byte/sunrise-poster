...
| `{holiday_length}` | 那段連假總共連休幾天 |

三個都是算整段連續休息日的，不是那個假日本身一天的——台灣人講
「中秋連假」，指的是連著的週休二日也算進去的那幾天（教師節是週一，
但連假從前面的週六就開始，是 3 天不是 1 天）。同一段連假裡有不只一個
具名假日時（例如中秋節接教師節），優先挑不是「補假」的當代表名字，
避免顯示成「補假連假」這種空洞的說法。單獨一天、沒接到週末的假日不算
連假，會被跳過繼續往後找下一個。

含 `holiday_*` 的樣板在查不到下一個連假時會直接跳過（記警告、不發回覆）。

已知的小瑕疵：代表名字是「這段連假裡最早出現的具名假日」，不一定是
最響亮的那個——例如農曆新年那個 9 天大長假，找到的代表名字會是
「小年夜連假」而不是「春節連假」，因為小年夜在整段假期最前面。
沒有做特別修正，機率不高且影響有限。
